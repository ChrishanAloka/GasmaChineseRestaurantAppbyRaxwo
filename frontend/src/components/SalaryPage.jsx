...
const SalaryPage = () => {
  ...
  const fetchSalaries = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("https://gasmachineserestaurantrms.onrender.com/api/auth/salaries", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  try {
    ...
    const res = await axios.post(
      "https://gasmachineserestaurantrms.onrender.com/api/auth/salary/add",
      payload,
      {
        headers: { Authorization: `Bearer ${token}` }
      }
