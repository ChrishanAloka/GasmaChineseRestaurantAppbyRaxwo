...
const KitchenLanding = () => {
  ...
  const fetchOrders = useCallback(async (initial = false) => {
    ...
    try {
      ...
      const res = await axios.get("https://gasmachineserestaurantappbyraxwo.onrender.com/api/auth/orders?limit=200", { // Large limit for ALL live orders
  ...
  const markAsReady = async (id) => {
    ...
    try {
      await axios.put(
        `https://gasmachineserestaurantappbyraxwo.onrender.com/api/auth/order/${id}/status`,
        { status: "Ready" },
        { headers: { Authorization: `Bearer ${token}` } }
      );
