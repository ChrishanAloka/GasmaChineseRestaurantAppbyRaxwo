...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchCurrency = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await axios.get("https://gasmachineserestaurantappbyraxwo.onrender.com/api/auth/settings/currency", {
  ...
  const updateCurrency = async (symbol, code) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        "https://gasmachineserestaurantappbyraxwo.onrender.com/api/auth/settings/currency",
